...
<!-- and use `destructure` from Flux to extract the parameters as `parameters_initial`.
...
(We could use `ComponentArrays` here and broadcast to avoid doing this, but this way avoids introducing an extra dependency.)
...
To interface with external libraries it is often desirable to use the [`StatefulLuxLayer`](https://lux.csail.mit.edu/stable/api/Lux/contrib#StatefulLuxLayer) to automatically handle the neural network states.
