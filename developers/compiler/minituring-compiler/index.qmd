...
```{julia}
using MacroTools, Distributions, Random, AbstractMCMC, FlexiChains
```
...
To make it easier to analyse the samples and compare them with results from Turing, additionally we define a version of `AbstractMCMC.bundle_samples` for our model and sampler that returns a `FlexiChains.VNChain` object of samples.
