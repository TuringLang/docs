...
- a latent variable whose value was computed from a model input before its tilde statement, since that value is about to be overwritten and the variable was probably meant to be an observation;
...
The overwrite check needs ForwardDiff to be loaded, and is best effort even then, so it can miss a dependency that runs through an untaken branch, a condition, or code it cannot differentiate.
...
The overwrite check catches a different kind of mistake, where a value derived from the data is given a prior instead of being observed:

```{julia}
@model function overwritten(y)
    z = 2y
    z ~ Normal()
    return y ~ Normal(z)
end

check_model(overwritten(1.5))
```

Here `z` is computed from `y` and then immediately discarded, because `z ~ Normal()` samples a fresh value for it.
Note that the check warns but still returns `true`, since this is legal Julia and legal DynamicPPL, just rarely what was meant.
