...
In a previous tutorial, we have discussed latent variable models, in particular [probabilistic principal component analysis (pPCA)]({{< meta probabilistic-pca >}}).
...
```{julia}
# Standard GPs don't scale very well in n, so we use a small subsample for
...
chain_ppca = sample(ppca, NUTS(; adtype=AutoMooncake()), 1000);
```
...
This is due to the non-linearities that we introduced, as without them the groups can be clearly distinguished using pPCA (see the pPCA tutorial).

Let's try the same with our linear kernel GPLVM model.

```{julia}
gplvm_linear = GPLVM_linear(dat[1:n_data, 1:n_features], ndim)
chain_linear = sample(gplvm_linear, NUTS(; adtype=AutoMooncake()), 500);
```
...
chain_gplvm = sample(gplvm, NUTS(; adtype=AutoMooncake()), 500);
```
