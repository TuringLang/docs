...
Now we extract the parameter samples from the sampled chain as `θ` (this is of size `2000 x 20` where `2000` is the number of iterations and `20` is the number of parameters). We'll use these primarily to determine how good our model's classifier is.

```{julia}
# Extract all weight and bias parameters.
θ = ch[@varname(parameters), chain=1, stack=true];
```
