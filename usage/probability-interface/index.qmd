...
The easiest way to manipulate and query Turing.jl models is via the DynamicPPL probability interface.
...
We recommend working with samples of type `DataStructures.OrderedDict` in this case (which Turing.jl re-exports, so can be used directly):
...
function cross_val(
    ...
    for (train, validation) in kfolds(dataset, nfolds)
        ...
        # For general models, however, typically samples will be generated using MCMC with Turing.jl.
