...
mean(chain[@varname(m)])
```

## Posterior predictive distribution

`chain[@varname(m)]` now contains samples from the posterior distribution of `m`.
...
```{julia}
predictive_model_with_mean_m = predictive_model | (; m = mean(chain[@varname(m)]))
...
::: {.callout-note}
`predict` returns a FlexiChain object itself, which will contain all the original parameters plus the newly predicted variables.
If you want to only include the newly predicted variables, you can use `predict(rng, predictive_model, chain; include_all=false)`.
:::
...
predicted_X = vec(predictive_samples[@varname(X), stack=true])
...
```{julia}
prior_predicted_X = vec(prior_predictive_samples[@varname(X), stack=true])
