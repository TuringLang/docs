...
For example, for the mean-field Gaussian family, we can use:
...
The main output of the algorithm is the `q` field, which is the variational approximation itself.
This represents the average of the parameters generated by the optimisation algorithm, and is computed using what is known as polynomial averaging[^SZ2013].
...
Usually, averaging will lead to better performance than simply using the last iterate.
If you want the last iterate, you can disable averaging in the algorithm:
...
The first argument to `KLMinRepGradDescent` (and the other `KLMin...` algorithms) is the AD backend used to compute the ELBO gradient.
We use `AutoForwardDiff()` above; for larger models a reverse-mode backend such as `AutoMooncake()` is often faster.
As with sampling, you must load the backend package yourself, for example `using Mooncake`.
...
To obtain summary statistics from VI, we can draw samples from the resulting variational approximation by calling `rand` on the `VIResult`.
Note that even though VI is often performed in transformed space, `rand(result)` will return samples in the original space of the parameters.
...
If you want to obtain samples in transformed space, you can access `result_fr.ldf` to obtain the `LogDensityFunction` used for inference.
Calling `rand(result_fr.ldf)` will return a vector of samples in the transformed space.
However, note that these vectors are harder to interpret and work with.
:::

Each sample is a [`DynamicPPL.VarNamedTuple`]({{< meta usage-varnamedtuple >}}), and a matrix of these can be converted into a `FlexiChain` object.

```{julia}
z = rand(result_fr, 100_000, 1)
using AbstractMCMC: AbstractMCMC
import FlexiChains
vi_chain = AbstractMCMC.from_samples(FlexiChains.VNChain, z)
```
...
vi_mean = Array(mean(vi_chain))
mcmc_mean = Array(mean(mcmc_chain))
...
function prediction(chain, x)
    α = mean(chain[@varname(intercept)])
    β = mean(chain[@varname(coefficients)])
    return α .+ x * β
end
```
...
vi_mf_chain = AbstractMCMC.from_samples(FlexiChains.VNChain, rand(result_mf, 10_000, 1))
vi_fr_chain = AbstractMCMC.from_samples(FlexiChains.VNChain, rand(result_fr, 10_000, 1))
```
...
```{julia}
Niters = FlexiChains.niters(vi_mf_chain)

preds_vi_mf = mapreduce(hcat, 1:5:Niters) do i
    return unstandardize(prediction(vi_mf_chain[iter=i], test), train_unstandardized.MPG)
end
...
preds_vi_fr = mapreduce(hcat, 1:5:Niters) do i
    return unstandardize(prediction(vi_fr_chain[iter=i], test), train_unstandardized.MPG)
end
...
Niters_mcmc = FlexiChains.niters(mcmc_chain)
preds_mcmc = mapreduce(hcat, 1:5:Niters_mcmc) do i
    return unstandardize(prediction(mcmc_chain[iter=i], test), train_unstandardized.MPG)
end
