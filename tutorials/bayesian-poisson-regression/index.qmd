...
# Import Plots and StatsPlots for visualisations and diagnostics.
...
```{julia}
using MCMCDiagnosticTools: gelmandiag
...
b0_exp = exp(mean(chain[@varname(b0), chain=1]))
b1_exp = exp(mean(chain[@varname(b1), chain=1]))
b2_exp = exp(mean(chain[@varname(b2), chain=1]))
b3_exp = exp(mean(chain[@varname(b3), chain=1]))
...
```{julia}
import FlexiChains
N = FlexiChains.niters(chain)

chains_new = chain[iter=201:N]
```
