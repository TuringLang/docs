...
let
    ...
    ess_arr = Array(ess(chain))
    @assert minimum(ess_arr) > 500 "Minimum ESS: $(minimum(ess_arr)) - not > 500"
    @assert mean(ess_arr) > 2_000 "Mean ESS: $(mean(ess_arr)) - not > 2000"
    @assert maximum(ess_arr) > 3_500 "Maximum ESS: $(maximum(ess_arr)) - not > 3500"
end
```
...
function prediction(chain, x)
    Niters = FlexiChains.niters(chain)
    intercept = chain[@varname(intercept), iter=200:Niters, chain=1]
    coefficients = chain[@varname(coefficients), iter=200:Niters, chain=1, stack=true]
    targets = intercept' .+ x * coefficients'
