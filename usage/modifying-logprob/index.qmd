...
Turing.jl accumulates log probabilities internally in an internal data structure that is accessible through the internal variable `__varinfo__` inside of the model definition.
To avoid users having to deal with internal data structures, Turing.jl provides the `Turing.@addlogprob!` macro which increases the accumulated log probability.
