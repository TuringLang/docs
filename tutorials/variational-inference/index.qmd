...
For computing `q_avg`, the default setting uses what is known as polynomial averaginge 
...
Since `AdvancedVI` does not implement a proximal operator for `Optimisers.Adam`, we must use the `AdvancedVI.ClipScale()` operator, which ensures that the scale matrix of the variational approximation is positive definite.
```{julia}
using Optimisers
...
Let's inspect the resulting variational approximation in more detail and compare against MCMC.
...
Also, the coverage of full-rank VI and MCMC is much better the the crude mean-field approximation.