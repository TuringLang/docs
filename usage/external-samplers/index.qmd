...
## Using External Samplers on Turing.jl Models

`Turing.jl` provides several wrapped samplers from external sampling libraries, e.g., HMC samplers from `AdvancedHMC`.
These wrappers allow new users to seamlessly sample statistical models without leaving `Turing.jl`
However, these wrappers might only sometimes be complete, missing some functionality from the wrapped sampling library.
Moreover, users might want to use samplers currently not wrapped within `Turing.jl`.

For these reasons, `Turing.jl` also makes running external samplers on Turing.jl models easy without any necessary modifications or wrapping!
...
In order to avoid type ambiguity within Turing.jl, at the moment it is necessary to declare `spl` as an external sampler to Turing.jl `espl = externalsampler(spl)`, where `externalsampler(s::AbstractMCMC.AbstractSampler)` is a Turing.jl function that types our external sampler adequately.
...
## Going beyond the Turing.jl API

As previously mentioned, the Turing.jl wrappers can often limit the capabilities of the sampling libraries they wrap.
...
However, the native HMC sampler within Turing.jl only allows the user to specify the type of the mass matrix despite the two options being possible within `AdvancedHMC`.
Thankfully, we can use Turing.jl's support for external samplers to define an HMC sampler with a custom mass matrix in `AdvancedHMC` and then use it to sample our Turing.jl model.
...
You can see an example of how to use `Pathfinder` with Turing.jl in [`Pathfinder`'s docs](https://mlcolab.github.io/Pathfinder.jl/stable/examples/turing/).

## Using new inference methods

So far we have used Turing.jl's support for external samplers to go beyond the capabilities of the wrappers.
We want to use this support to employ a sampler not supported within Turing.jl's ecosystem yet.
...
Using this as well as other inference methods outside the Turing.jl ecosystem is as simple as executing the code shown below:
...
As previously stated, in order to use external sampling libraries within `Turing.jl` they must follow the `AbstractMCMC` API.
...
This allows `Turing.jl` to seamlessly extract the parameter values at each step of the chain when bundling the chains.
Note that if the external sampler produces transitions that Turing.jl cannot parse, the bundling of the samples will be different or fail.
