...
This implies that the distribution will reflect less uncertainty about the probability of receiving a heads and the plot will become more tightly centred around 0.5 for a large enough number of coin flips.
...
Additionally, we load `FlexiChains`, a library for analysing and visualising the samples with which we approximate the posterior distribution.
...
```{julia}
#| echo: false
@assert isapprox(mean(chain[@varname(p)]), 0.5; atol=0.1) "Estimated mean of parameter p: $(mean(chain[@varname(p)])) - not in [0.4, 0.6]!"
```
