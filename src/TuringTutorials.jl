...
function weave(
    ...
    build::Tuple{Vararg{Symbol}}=DEFAULT_BUILD,
)
    if !issubset(build, (:script, :html, :pdf, :github, :notebook))
        throw(
            ArgumentError(
                "only build types :script, :html, :pdf, :github, and :notebook are supported",
            ),
        )
    end
    ...
    if :html in build
        ...
        mkpath(dir)
        Weave.weave(
            target; doctype="md2html", out_path=dir, args=args, css=CSS_FILE, fig_ext=".svg"
        )
    end
    if :pdf in build
        ...
        try
            Weave.weave(
                target; doctype="md2pdf", out_path=dir, template=LATEX_FILE, args=args
            )
        catch ex
