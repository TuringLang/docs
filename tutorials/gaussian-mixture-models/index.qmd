...
let
    ...
    for i in FlexiChains.chain_indices(chains)
        μ_mean = vec(mean(chains[@varname(μ), chain=i, stack=true]; dims=1))
...
```{julia}
plot(chains[[@varname(μ[1]), @varname(μ[2])]]; legend=true)
```
...
```{julia}
plot(chains[[@varname(w[1]), @varname(w[2])]]; legend=true)
```
...
μ_mean = [mean(chains[@varname(μ[i])]) for i in 1:2]
w_mean = [mean(chains[@varname(w[i])]) for i in 1:2]
...
```{julia}
assignments = [mean(chains[@varname(k[i])]) for i in 1:N]
scatter(
    x[1, :],
    x[2, :];
    legend=false,
